#!/usr/bin/env python3
"""
emess_rss.py — בונה ומעדכן פיד RSS לתוכנית "התעוררות" (מזהה 437 ב-tax_broadcasters)
באתר emess.co.il, ישירות מתוך ה-REST API הפתוח של וורדפרס.
...
מבנה שאומת מול נתונים אמיתיים מהאתר:
  ...
  - כל פרק מתויג ב-taxonomy בשם tax_broadcasters עם ID של התוכנית שלו
    (עבור "התעוררות" - ה-ID הוא 437)
  - ניתן לסנן ישירות: /wp-json/wp/v2/aryo_programs?tax_broadcasters=437
  - קובץ השמע של כל פרק נמצא בשדה audio_in_content[0].audio_file

הרצה:
    pip install requests
    python3 emess_rss.py
"""
...
TERM_ID = 437                     # מזהה התוכנית "התעוררות"
OUTPUT_XML = "feed.xml"
